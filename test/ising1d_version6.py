...
class TFI_1d:
    ...
    def local_E_heisenberg(self):
        ...
        for i in range(self.L-1):
            temp.append(self.build_TFI_1d(3/2 * tf.concat([tf.concat([self.x[:i], 
                    tf.transpose(tf.matmul(tf.convert_to_tensor([[1.,0.], [0., -1.]]) ,(self.x[i])[:, tf.newaxis])),
                    tf.transpose(tf.matmul(tf.convert_to_tensor([[1.,0.], [0., -1.]]) ,(self.x[i+1])[:, tf.newaxis])),
                    self.x[i+2:]], axis=0),
                                          tf.concat([self.x[:i], 
                    tf.transpose(tf.matmul(0.5 * tf.convert_to_tensor([[0.,1.], [0., 0.]]) ,(self.x[i])[:, tf.newaxis])),
                    tf.transpose(tf.matmul(tf.convert_to_tensor([[0.,0.], [1., 0.]]) ,(self.x[i+1])[:, tf.newaxis])),
                    self.x[i+2:]], axis=0),
                                          tf.concat([self.x[:i], 
                    tf.transpose(tf.matmul(0.5 * tf.convert_to_tensor([[0.,0.], [1., 0.]]) ,(self.x[i])[:, tf.newaxis])),
                    tf.transpose(tf.matmul(tf.convert_to_tensor([[0.,1.], [0., 0.]]) ,(self.x[i+1])[:, tf.newaxis])),
                    self.x[i+2:]], axis=0)],
                    axis=0)))
            
            num_temp = temp[i][0][0]
            self.localE -= 2/3 * self.j * num_temp / num_oldAmp
            self.loss -= (temp[i] - oldAmp)

        temp.append(self.build_TFI_1d(3/2 * tf.concat([tf.concat([
                    tf.transpose(tf.matmul(tf.convert_to_tensor([[1.,0.], [0., -1.]]) ,(self.x[0])[:, tf.newaxis])),
                    self.x[1:self.L-1], 
                    tf.transpose(tf.matmul(tf.convert_to_tensor([[1.,0.], [0., -1.]]) ,(self.x[self.L-1])[:, tf.newaxis])),
                    ], axis=0),
                                          tf.concat([
                    tf.transpose(tf.matmul(0.5 * tf.convert_to_tensor([[0.,1.], [0., 0.]]) ,(self.x[0])[:, tf.newaxis])),                          
                    self.x[1:self.L-1], 
                    tf.transpose(tf.matmul(tf.convert_to_tensor([[0.,0.], [1., 0.]]) ,(self.x[self.L-1])[:, tf.newaxis])),
                    ], axis=0),
                                          tf.concat([
                    tf.transpose(tf.matmul(0.5 * tf.convert_to_tensor([[0.,0.], [1., 0.]]) ,(self.x[0])[:, tf.newaxis])),                          
                    self.x[1:self.L-1], 
                    tf.transpose(tf.matmul(tf.convert_to_tensor([[0.,1.], [0., 0.]]) ,(self.x[self.L-1])[:, tf.newaxis])),
                    ], axis=0)],
                    axis=0)))
